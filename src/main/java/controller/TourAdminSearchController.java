...
public class TourAdminSearchController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			response.sendRedirect("admin_tours?errCode=slotNumberError");
			return;
		} catch (Exception ex) {
		...
		try {
			...
			if (adminFilter_priceF > adminFilter_priceT) {
				response.sendRedirect("admin_tours?errCode=priceNumberError");
				return;
			}
		...
		if (adminFilter_slot != -1) {
			...
			response.sendRedirect("admin_tours?errCode=noFilter");
			return;
		}

		request.setAttribute("mgmtTours", result);
		RequestDispatcher dispatcher = request.getRequestDispatcher("admin_tours");
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			response.sendRedirect("admin_tours?errCode=tourActiveError");
			return;
		} catch (Exception ex) {
		...
		if (tourid_active != -1) {
			...
			if (affectedRows == 1) {
				response.sendRedirect("admin_tours?message=successfullyUpdated");
				return;
			} else {
				response.sendRedirect("admin_tours?errCode=unsuccessfulUpdate");
				return;
			}
		}
