...
<body class='p-2'>
	...
	if(sessCart != null){
		...
		<div class='paymentDetails col-3'>
			<div class='cartTotal p-2 card bg-teal-100'>
                <div class='subtotal'>
                    <h6>Subtotal</h6>
                    <h6 id="subtotal">S$<%=String.format("%.2f", subtotal) %></h6>
                </div>
                <hr>
                <div class='subtotal mt-2'>
                    <h6>GST</h6>
                    <h6 id="gst">S$<%=String.format("%.2f", gst)%></h6>
                </div>
                <hr>
                <div class='subtotal mt-2'>
                    <h4>
                        ...
                        <h4 id="checkoutPrice">S$<%=String.format("%.2f", checkoutPrice) %></h4>
                </div>
			</div>
			...
			<div class='card bg-teal-100 p-2'>
				<form action="/assignment2_jad_client/StripeCheckout" method="post">
					<input type="submit" class='w-100p my-1 bg-yellow-300' value="PROCEED"/>
				</form>
			</div>
		</div>
	</div>
	...
	<div class='shoppingCart card row p-2 h-90p'>
		...
		<form class='paymentDetails col-3' submit='#'>
			...
			<div class='card bg-teal-100 p-2'>
				<button class='w-100p my-1 bg-yellow-300' disabled>PROCEED</button>
			</div>
		</form>
	</div>
